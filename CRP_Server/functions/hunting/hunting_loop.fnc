...

{
	...
	if (count _animalVariants == 0) exitWith {};
	_currentAnimal = _animalVariants select floor (random (count _animalVariants));
		...
		_animal = _animalVariants select floor (random (count _animalVariants));
