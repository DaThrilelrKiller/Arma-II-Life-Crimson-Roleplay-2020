...
echo [BUILD] Generating mission.template from vehicle classes...

:: Create PowerShell script to generate mission.template
set "TEMPLATE_PS_SCRIPT=%TEMP%\generate_template.ps1"
set "BIEDI_FILE=%PROJECT_ROOT%\CRP_Server\mission.biedi"
...
echo.
echo     # Extract NAME
echo     $nameMatch = [regex]::Match^(^$argsContent, 'NAME\s*=\s*"([^"]*)"'^)
echo     if ^($nameMatch.Success^) {
echo         $name = $nameMatch.Groups[1].Value
echo     }
echo.
echo     # Extract TYPE
echo     $typeMatch = [regex]::Match^(^$argsContent, 'TYPE\s*=\s*"([^"]*)"'^)
echo     if ^($typeMatch.Success^) {
echo         $type = $typeMatch.Groups[1].Value
echo     }
echo.
echo     # Extract POSITION
echo     $posMatch = [regex]::Match^(^$argsContent, 'POSITION\s*=\s*"([^"]*)"'^)
echo     if ^($posMatch.Success^) {
echo         $position = $posMatch.Groups[1].Value
echo     }
echo.
echo     # Extract AZIMUT
echo     $azimutMatch = [regex]::Match^(^$argsContent, 'AZIMUT\s*=\s*"([^"]*)"'^)
echo     if ^($azimutMatch.Success^) {
echo         $azimut = $azimutMatch.Groups[1].Value
echo     }
echo.
echo     # Parse position array - remove brackets and split, then rejoin
echo     $posStr = $position -replace '^\s*\[\s*', '' -replace '\s*\]\s*$', ''
echo     $posParts = $posStr -split '\s*,\s*'
echo     $posArray = '[' + ^($posParts -join ','^) + ']'
echo.
echo     # Ensure azimut is a number
echo     try {
echo         $azimutNum = [double]::Parse^(^$azimut^)
echo     } catch {
echo         $azimutNum = 0
echo     }
echo.
